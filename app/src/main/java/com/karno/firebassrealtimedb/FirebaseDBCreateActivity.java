...
import com.karno.firebassrealtimedb.model.barang;
...
// mengambil referensi ke Firebase Database
        mFirebaseDatabase=FirebaseDatabase.getInstance().getReference("dbcreate");
 ...
 */
mFirebaseDatabase.child("DBcreate") //akses parent index, ibaratnya seperti nama tabel
.child(Barang.getKey()) //select barang berdasarkan key
                .setValue(Barang) //set value barang yang baru
                .addOnSuccessListener(this, new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
/**STT Pelita Bangsa 2018
 By Donny Maulana
 16
 * Baris kode yang akan dipanggil apabila proses
 update barang sukses
 */
Snackbar.make(findViewById(R.id.bt_submit), "Data berhasil diupdatekan", Snackbar.LENGTH_LONG).setAction("Oke", new
 ...
 */
        mFirebaseDatabase.child("barang").push().setValue(Barang).addOnSuccessListener(this, new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        etNama.setText("");
                        etMerk.setText("");
